...
training_indices = np.random.choice(range(num_subjects), num_training, replace = False)
#training_indices = range(num_training)
#validation_indices = range(num_training,num_subjects)
validation_indices = np.setdiff1d(range(180), training_indices)
...
np.savetxt('./loss/T7_try_again_l_h.txt', numpy_loss_history, delimiter=",")

#saving validation loss logs
val_loss_history = history_callback.history["val_loss"]
numpy_val_loss_history = np.array(val_loss_history)
np.savetxt('./loss/T7_try_again_v_l_h.txt',numpy_val_loss_history, delimiter=",")

#saving predictions
np.savetxt('./class_preds/T7_try_again_class_pred.txt',y_pred, delimiter=",")

#I also want to save the models
json_filename = './jsons/T7_try_again.json'
weights_filename = './weights/T7_try_again_w.h5'
