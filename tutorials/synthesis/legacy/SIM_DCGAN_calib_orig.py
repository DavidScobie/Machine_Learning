...
if flag_randomHyperParam:
    ...
    learning_rate_G = np.random.choice([1e-4, 2e-4, 5e-4])
    learning_rate_D = np.random.choice([1e-4, 2e-4, 5e-4])
...
print('learning_rate_G: %s' % learning_rate_G, flush=True, file=fid_output_info)
print('learning_rate_D: %s' % learning_rate_D, flush=True, file=fid_output_info)
...
def generator(z, y):  # conditioned dc-generator
    ...
    if generator_shortcuts[0]:
        g_h1p = tf.concat([g_h1p, tf.image.resize_bilinear(y, data_size_4)], axis=3)
    ...
    if generator_shortcuts[1]:
        g_h2t = tf.concat([g_h2t, tf.image.resize_bilinear(y, data_size_3)], axis=3)
    ...
    if generator_shortcuts[2]:
        g_h3t = tf.concat([g_h3t, tf.image.resize_bilinear(y, data_size_2)], axis=3)
    ...
    if generator_shortcuts[3]:
        g_h4t = tf.concat([g_h4t, tf.image.resize_bilinear(y, data_size_1)], axis=3)
    ...
    if generator_shortcuts[4]:
        g_h5t = tf.concat([g_h5t, y], axis=3)
...
def discriminator(x, y):  # x = tf.placeholder(tf.float32, [miniBatch_size]+data_size+[1])
    # initial resnet
    d_h1s = tf_tools.nn_leaky_relu(tf.nn.conv2d(tf.concat([x, y], axis=3), D_W1s, strides_none, "SAME") + D_b1s)  # NB. no batch_norm for true mean and scale
...
# normal GAN-losses
tf.add_to_collection('d_losses', tf.reduce_mean(tf.nn.sigmoid_cross_entropy_with_logits(logits=D_logit_real, labels=tf.scalar_mul(.9, tf.ones_like(D_logit_real)))))
tf.add_to_collection('d_losses', tf.reduce_mean(tf.nn.sigmoid_cross_entropy_with_logits(logits=D_logit_fake, labels=tf.zeros_like(D_logit_fake))))
if lambda_supervised >= 0:
    tf.add_to_collection('g_losses', tf.reduce_mean(tf.nn.sigmoid_cross_entropy_with_logits(logits=D_logit_fake, labels=tf.ones_like(D_logit_fake))))

# optional conditioner loss here

G_loss = tf.add_n(tf.get_collection('g_losses'))
G_train_op = tf.train.AdamOptimizer(learning_rate=learning_rate_G, beta1=0.5).minimize(G_loss, var_list=theta_G)
D_loss = tf.add_n(tf.get_collection('d_losses'))
D_train_op = tf.train.AdamOptimizer(learning_rate=learning_rate_D, beta1=0.5).minimize(D_loss, var_list=theta_D)
